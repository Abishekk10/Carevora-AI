...
export default function JobSearch() {
  ...
  const [addToAppSuccess, setAddToAppSuccess] = useState(null);
  ...
  const addToApplications = async (job) => {
    try {
      ...
      setAddToAppSuccess(`Added "${job.title}" at ${job.company} to applications`);
      setTimeout(() => setAddToAppSuccess(null), 3000);
    } catch (err) {
      setError(err.message || "Failed to add job to applications");
    }
  };
    ...
    <section className="animate-fade-up">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="page-subtitle">Search live opportunities from JobPilot's provider network.</p>
        </div>
        {hasSearched && !isLoading && (
          <p className="text-sm font-medium text-slate-500">{jobs.length} opportunities found</p>
        )}
      </div>
      ...
      <form onSubmit={submitSearch} className="surface mt-7 p-4 sm:p-5">
        <div className="grid gap-3 lg:grid-cols-[1.4fr_1fr_170px_140px_auto]">
          ...
          <label className="relative">
            <MapPin className="pointer-events-none absolute left-3.5 top-3 h-5 w-5 text-slate-400" />
            <input 
              className="field pl-11" 
              value={form.location} 
          ...
          <label className="relative">
            ...
            <select 
              ...
              onChange={(event) => setForm({ ...form, results_per_page: Number(event.target.value) })} 
          ...
          <button className="btn-primary" disabled={isLoading || !form.query.trim()}>
        ...
        <div className="mt-6 space-y-4">
          {jobs.map((job) => (
            <JobCard 
              ...
              job={job} 
              onMatchResume={matchResume} 
              onAddToApplications={addToApplications} 
            />
          ))}
          {hasSearched && !error && jobs.length === 0 && (
            <div className="surface py-16 text-center">
              <Search className="mx-auto h-9 w-9 text-slate-300" />
              <h2 className="mt-4 font-semibold text-slate-800">No roles matched this search</h2>
              <p className="mt-1 text-sm text-slate-500">Try a broader job title or a different location.</p>
            </div>
          )}
          {!hasSearched && (
            <div className="surface mt-6 py-16 text-center">
              <Search className="mx-auto h-9 w-9 text-indigo-300" />
              <h2 className="mt-4 font-semibold text-slate-800">Start with a focused search</h2>
              <p className="mt-1 text-sm text-slate-500">Use a role, skill, or company name to discover current openings.</p>
            </div>
          )}
        </div>
      )}

      {matchState && <ResumeMatchModal {...matchState} onClose={() => setMatchState(null)} />}
    </section>
  );
}
